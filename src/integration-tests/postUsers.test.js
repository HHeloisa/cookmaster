...
const { usersMessages } = require('../messages');
...
describe.only('Valida a criação de um usuário em post /users', () => {
  ...
  describe('Erro no cadastro de usuário', () => {
    ...
    before(async () => {
      ...
      connectionMock.db('Cookmaster').collection('users').insertOne(userMock);
    });
    ...
    it('Retorna erro ao tentar criar um usuário que já existe', async () => {
      ...
      expect(response.body).to.have.property('message');
      expect(response.body.message).to.be.equal(usersMessages.emailNotUnic);
    })
  }); 
});